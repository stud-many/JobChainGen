...
def check_def_case():
    ActiveSheet = model.Sheets.getByIndex(0)
    defTemplatekCaseFile = ActiveSheet.getCellRangeByName("B2").String
    if defTemplatekCaseFile[-3:].lower() == "def":
        return os.path.isfile(defTemplatekCaseFile)
    else:
        return False
...
def make_Jobchain():
    ...
    SubJobListRAWTXT = ActiveSheet.getCellRangeByName("B8").String
    ...
    simnames = [JobChainName + "_" + i[3] for i in BoundaryConditions]
    sh_names = [i+".sh" for i in simnames]
    ccl_names = [i+".ccl" for i in simnames]
    def_names = [i+".def" for i in simnames]
    dependencies = [i[1] for i in BoundaryConditions]
    maxiterrations = [int(float(i[2])) for i in BoundaryConditions]
    #BoundaryConditions = [i for i in BoundaryConditions[-3:]

    Expressions = Expressions[4:] #Ersten Columns aussortieren
    Units = Units[4:]
    BoundaryConditions = [i[4:] for i in BoundaryConditions]

    print_out("creating ccl-files and submission scripts...")
    
    jobchainTXT = ""        
    sh_Jobnames = (" ".join([l for l in sh_names]))
    sh_Jobdependencies = (" ".join([str(int(float(l))) for l in dependencies]))
    SubJobListTXT = SubJobListRAWTXT.replace("BASHNAMES",sh_Jobnames)
    SubJobListTXT = SubJobListTXT.replace("DEPENDENCIES",sh_Jobdependencies)
    SubJobListTXT = SubJobListTXT.replace("JOBIDS"," ".join([str(l) for l in range(len(dependencies))]))
    jobchainTXT += SubJobListTXT
